...
const Task = forwardRef(({ id, notes, index, removeTask }, ref) => {
  ...
  const onMouseEnter = useCallback(() => setShowOptions(true), []);
  ...
  const deleteTask = useCallback(() => {
    setDeleting(true);
    axios
      .delete(`/server/freelance_function/${id}`) // Use backticks and proper path
      .then((response) => {
        const {
          data: {
            todoItem: { id }
  ...
  return (
    <div className="task" ref={ref} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
...
function App() {
  ...
  useEffect(() => {
    if (fetchState !== 'fetched') {
      axios
        .get('/server/freelance_function/all', {
  ...
  const createTodo = useCallback(
    (event) => {
      ...
      setSubmitting(true);
      axios
        .post('/server/freelance_function/add', { notes })
        .then((response) => {
          const {
            data: { todoItem }
          } = response.data;
